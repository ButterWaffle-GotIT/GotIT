...
const tagData = [
    ...
    { name: '프론트엔드', color: 'bg-cyan-400', IconComponent: CategoryFrontendIcon },
    { name: '백엔드', color: 'bg-green-600', IconComponent: CategoryBackendIcon },
    { name: 'UI/UX', color: 'bg-rose-400', IconComponent: CategoryUiuxIcon },
    ...
    { name: '보안/네트워크', color: 'bg-orange-400', IconComponent: CategorySecurityIcon },
    { name: 'DevOps', color: 'bg-amber-400', IconComponent: CategoryDevopsIcon },
    { name: 'IT비즈니스', color: 'bg-blue-400', IconComponent: CategoryBusinessIcon },
];

const renderTag = (tag: typeof tagData[0], selectedTag: string, onTagSelect: (name: string) => void) => {
    ...
    return (
        <div
            ...
            className={`
                px-5 py-2 rounded-xl outline outline-[0.25px] outline-offset-[-0.25px] inline-flex justify-center items-center gap-2 cursor-pointer
                ${finalClasses}
            `}
        >
            {/* 아이콘 컨테이너 */}
            <div className={`w-6 p-1 ${tag.color} rounded-[100px] outline outline-[0.25px] outline-offset-[-0.25px] outline-white flex justify-start items-center gap-2.5 overflow-hidden`}>
            ...
            <div className="flex justify-start items-center gap-0.5">
                <span className="justify-start text-white text-sm font-bold font-['Pretendard'] leading-6">#</span>
                <span className="justify-start text-white text-sm font-bold font-['Pretendard'] leading-6">{tag.name}</span>
            </div>
        </div>
    );
};
